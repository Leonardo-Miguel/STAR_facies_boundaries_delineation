...
def load_subset(data_file:str, label_file:str, section_type:str, partition:str, samples_partition:list, pred_file:str=None):
    ...
    if partition != 'test':
        if section_type == 'inline':
            ...
            subset_data = data[:, samples_partition, :]
            subset_data = np.transpose(subset_data, (1, 0, 2))
            if label_file is not None:
                subset_label = label[:, samples_partition, :]
                subset_label = np.transpose(subset_label, (1, 0, 2))
            if pred_file is not None:
                subset_pred = pred[:, samples_partition, :]
                subset_pred = np.transpose(subset_pred, (1, 0, 2))
                
        elif section_type == 'timeslice':
            subset_data = data[:, :, samples_partition]
            subset_data = np.transpose(subset_data, (2, 0, 1))
            if label_file is not None:
                subset_label = label[:, :, samples_partition]
                subset_label = np.transpose(subset_label, (2, 0, 1))
            if pred_file is not None:
                subset_pred = pred[:, :, samples_partition]
                subset_pred = np.transpose(subset_pred, (2, 0, 1))
